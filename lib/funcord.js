...
class Discord {
	...
	getChannelLogs(id, limit, settings, callback) {
		var before = settings.before;
		var after = settings.after;
		var around = settings.around;
		this.callAPI({method: 'GET', uri: '/channels/'+id+'/messages', body: {limit: limit, before: before, after: after, around: around}, function(res) {
			if (callback) callback(res);
		})
	}
    ...
    getChannelLogs(channel, options, callback) {
    	...
    	if (options.alimit) uri += '&limit='+options.limit;
		this.callAPI({method: 'GET', uri: uri}, function(res) {
			if (callback) callback(res);
		})
	}
}
